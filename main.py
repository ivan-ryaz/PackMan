...
class Ghost(Entity):
    ...
    def update(self, dt):
        ...
        self.mode.update(dt)
        if self.mode.cur is scat:
            self.scatter()
        elif self.mode.cur is chasee:
            self.chase()
...
class MainMode(object):
    ...
    def update(self, dt):
        ...
        if self.timer >= self.time:
            if self.mode is scat:
                self.chase()
            elif self.mode is chasee:
                self.scatter()
...
class ModeController(object):
    ...
    def update(self, dt):
        ...
        if self.cur is freightt:
            self.timer += dt
            if self.timer >= self.time:
                self.time = None
                self.entity.normalMode()
                self.cur = self.mainmode.mode
        elif self.cur in [scat, chasee]:
            self.cur = self.mainmode.mode
        if self.cur is spawn_:
            if self.entity.node == self.entity.spawnNode:
                self.entity.normalMode()
                self.cur = self.mainmode.mode

    def setFreightMode(self):
        if self.cur in [scat, chasee]:
            self.timer = 0
            self.time = 7
            self.cur = freightt
        elif self.cur is freightt:
            self.timer = 0
...
class levelData(object):
    def __init__(self):
        self.obj = None
        self.leveldict = {0: level1, 1: level2}

    def loadlevel(self, level):
        self.obj = self.leveldict[level % len(self.leveldict)]()
...
class NodeG(object):
    ...
    def createHomeNodes(self, xoffset, yoffset):
        homedata = numpy.array([['X', 'X', '+', 'X', 'X'], ['X', 'X', '.', 'X', 'X'],
                                ['+', 'X', '.', 'X', '+'], ['+', '.', '+', '.', '+'],
                                ['+', 'X', 'X', 'X', '+']])
...
class Spritesheet(object):
    def __init__(self):
        ...
        width = int(self.sheet.get_width() / btiwidth * twidth)
        height = int(self.sheet.get_height() / btiheignt * theight)
...
class PacmanSprites(Spritesheet):
    ...
    def defineAnimations(self):
        ...
        self.animations[dth] = Animator(((0, 12), (2, 12), (4, 12), (6, 12), (8, 12), (
    ...
    def update(self, dt):
        if self.entity.alive == True:
            ...
            self.entity.image = self.getImage(*self.animations[dth].update(dt))
...
class GhostSprites(Spritesheet):
    ...
    def update(self, dt):
        ...
        if self.entity.mode.cur in [scat, chasee]:
            if self.entity.direction == left:
        ...
        elif self.entity.mode.cur == freightt:
            self.entity.image = self.getImage(10, 4)
        elif self.entity.mode.cur == spawn_:
            if self.entity.direction == left:
...
class Text(object):
    def __init__(self, text, color, x, y, size, time=None, id=None, visible=True):
        ...
        self.label = None
        self.destroy = False
        self.setupFont('fonts/start.ttf')
        self.createLabel()
...
class TextGroup(object):
    ...
    def setupText(self):
        size = theight
        self.alltext[scoret] = Text('0'.zfill(8), (255, 255, 255), 0, theight, size)
        self.alltext[levelt] = Text(str(1).zfill(3), (255, 255, 255), 23 * twidth, theight, size)
        self.alltext[readyt] = Text('READY!', (255, 255, 0), 11.25 * twidth, 20 * theight, size, visible=False)
        self.alltext[pauset] = Text('PAUSED!', (255, 255, 0), 10.625 * twidth, 20 * theight, size, visible=False)
        self.alltext[got] = Text('GAMEOVER!', (255, 255, 0), 10 * twidth, 20 * theight, size, visible=False)
    ...
    def hideText(self):
        self.alltext[readyt].visible = False
        self.alltext[pauset].visible = False
        self.alltext[got].visible = False

    def newScore(self, score):
        self.updateText(scoret, str(score).zfill(8))

    def updateLevel(self, level):
        self.updateText(levelt, str(level + 1).zfill(3))
...
class levelSprites(Spritesheet):
    def __init__(self, levelfile, rotfile):
        Spritesheet.__init__(self)
        self.data = self.readmz(levelfile)
        self.rotdata = self.readmz(rotfile)

    def getImage(self, x, y):
        return Spritesheet.getImage(self, x, y, twidth, theight)

    def readmz(self, levelfile):
        return numpy.loadtxt(levelfile, dtype='<U1')
...
class GameCntrl(object):
    ...
    def setBG(self):
        ...
        self.bgf.fill((0, 0, 0))
        self.bgn = self.levelsprites.consBG(self.bgn, self.level % 5)
        self.bgf = self.levelsprites.consBG(self.bgf, 5)
        self.flashBG = False
        self.bg = self.bgn

    def start(self):
        self.mzdata.loadlevel(self.level)
        self.levelsprites = levelSprites('levels/' + self.mzdata.obj.name + '.txt',
                                       'levels/' + self.mzdata.obj.name + '_rotation.txt')
        self.setBG()
    ...
    def starto(self):
        self.mzdata.loadlevel(self.level)
        self.levelsprites = levelSprites('levels/level1.txt', 'levels/level1_rotation.txt')
        self.setBG()
        self.nodes = NodeG('levels/level1.txt')
        self.nodes.setPortalPair((0, 17), (27, 17))
        homekey = self.nodes.createHomeNodes(11.5, 14)
        self.nodes.conHomnod(homekey, (12, 14), left)
        self.nodes.conHomnod(homekey, (15, 14), right)
        self.pacman = Pacman(self.nodes.getNodeFromTiles(15, 26))
        self.pellets = PelletGroup('levels/level1.txt')
        self.ghosts = GhostGroup(self.nodes.getStartTempNode(), self.pacman)
    ...
    def ghostE(self):
        for ghost in self.ghosts:
            if self.pacman.collideGhost(ghost):
                ...
                elif ghost.mode.cur is not spawn_:
                    if self.pacman.alive:
                        self.lives -= 1
    ...
    def fruitE(self):
        ...
        if self.fruit is not None:
            if self.pacman.collideCheck(self.fruit):
                ...
                self.TG.addText(str(self.fruit.points), (255, 255, 255), self.fruit.position.x, self.fruit.position.y,
                                8,
                                time=1)
                fruitsc = False
